...
/***************************************RESERVA**********************************************/
   ...
   public void Insert_r(Reserva itens){
                  
    MySqlConnection conexao = new MySqlConnection(enderecoConexao);

    conexao.Open();

    string sqlInsert = "INSERT INTO reserva (nome, quantidade, preco, data_reserva) VALUES (@nome, @quantidade, @preco, @data_reserva)";

    MySqlCommand comando = new MySqlCommand(sqlInsert, conexao);

    comando.Parameters.AddWithValue("@nome", itens.nome);
    comando.Parameters.AddWithValue("@quantidade", itens.quantidade);
    comando.Parameters.AddWithValue("@preco", itens.preco);
    comando.Parameters.AddWithValue("@data_reserva", itens.data_reserva);
    ...
    public List<Reserva> Listar_Reservas(){
        ...
        string sqlList = "SELECT * FROM reserva ORDER BY nome";
        ...
        reserva.id = dados.GetInt32("id_reserva");

        if(!dados.IsDBNull(dados.GetOrdinal("nome")))
        reserva.nome = dados.GetString("nome");

        if(!dados.IsDBNull(dados.GetOrdinal("quantidade")))
        reserva.quantidade = dados.GetInt32("quantidade");

        if(!dados.IsDBNull(dados.GetOrdinal("preco")))
        reserva.preco = dados.GetDouble("preco");

        if(!dados.IsDBNull(dados.GetOrdinal("data_reserva")))
        reserva.data_reserva = dados.GetString("data_reserva");
    ...
    /*ALTERAR RESERVA*/ 
    public void Alterar_r(Reserva itens){

    MySqlConnection conexao = new MySqlConnection(enderecoConexao);

    conexao.Open();

    string sqlUpdate = "UPDATE reserva set nome = @nome, quantidade = @quantidade, preco = @preco, data_reserva = @data_reserva WHERE id_reserva = @id";        
    ...
    /*EXCLUIR RESERVA*/
     public void Excluir_r(int id){
    
    MySqlConnection conexao = new MySqlConnection(enderecoConexao);
    
    conexao.Open();

    string sqlDelete = "DELETE FROM reserva WHERE id_reserva = @id";
    ...
    /*RETORNO DE RESERVA*/
     public Reserva RetornoReserva(int id){
    ...
    string sqlConsulta = "SELECT * FROM reserva WHERE id_reserva = @id";

    MySqlCommand comando = new MySqlCommand(sqlConsulta, conexao);

    comando.Parameters.AddWithValue("@id", id_itens);
